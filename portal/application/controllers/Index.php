...
class Index extends App_Controller
{
    ...
    public function sendMessage()
    {
        ...
        $this->form_validation->set_rules('name', _l('client_name'), 'required');
        $this->form_validation->set_rules('email', _l('client_email'), 'trim|required|valid_email');
        $this->form_validation->set_rules('subject', _l('subject'), 'trim|required');
        $this->form_validation->set_rules('organization', _l('subject'), 'trim|required');
        $this->form_validation->set_rules('message', _l('message'), 'trim|required');

        if ($this->form_validation->run() !== false) {
            $post_data  = $this->input->post();
           
         foreach (['info@thelegalclinics.com','rmahmoud@thelegalclinics.com','abdulrahman.g.alabod@gmail.com'] as $email){
            $this->sendEmailToOne($email, $post_data['subject'], ($post_data['message'] .
            "<br>Organization: ".$post_data['organization'] ."<br>Name: ".$post_data['name']."<br>Email: ".$post_data['email'])
           );
           usleep(500);
    ...
    private  function sendEmailToOne($toemail, $subjecttxt, $emailtxt)
    {
...
class AliEmail
{
    static private $smtpserver = 'smtpdm-ap-southeast-1.aliyun.com';
    static private $smtpuser = 'sender@send.atriumadv.com';
    static private $smtpsecret = 'PassW0rd149858';
    static private $smtpserverport  = 25;
    static public function sendEmailBySMTP($toemail, $subjecttxt, $emailtxt)
    {
        $smtp = new xsmtp(self::$smtpserver, self::$smtpserverport, true, self::$smtpuser, self::$smtpsecret);
...
class xsmtp
{
    ...
    {
        //function xsmtp($relay_host = "", $smtp_port = 25, $auth = false, $user, $pass){
        $this->debug      = false;
